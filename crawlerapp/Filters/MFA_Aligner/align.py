...
MFA_ALIGNER = os.path.join(CONFIG_PATH,'Filters/MFA_Aligner/montreal-forced-aligner/bin/')
G2P_MODEL = os.path.join(CONFIG_PATH,'Filters/MFA_Aligner/spanish_g2p.zip')
LANG_MODEL = os.path.join(CONFIG_PATH,'Filters/MFA_Aligner/spanish.zip')
...
def align(video_id,lang):
    os.mkdir(video_id)
    work_dir = os.path.join(CRAWLED_VIDEOS_ABSOLUTE, video_id)
    ...
    output_h5py(word_intervals, video_id, "words")
    output_h5py(word_intervals, video_id, "phones")
